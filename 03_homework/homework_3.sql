...
SELECT vendor_id, COUNT(*) AS number_of_booth_rentals
...
INNER JOIN customer AS c ON cp.customer_id = c.customer_id
...
CREATE TEMPORARY TABLE temp.new_vendor (
  ...
  vendor_owner_first_name VARCHAR(255) NOT NULL,
  vendor_owner_last_name VARCHAR(255) NOT NULL
);
...
FROM (
  ...
  FROM customer_purchases
) AS purchase_data  -- Using the previous query
WHERE purchase_month = '04' AND purchase_year = '2019'  -- Filtering for April 2019
